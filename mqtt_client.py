...
def main():
    ...
    parser.add_argument(
        "--broker", default=DEFAULT_BROKER, help="MQTT Broker IP/Hostname"
    )
    parser.add_argument(
        "--port", type=int, default=DEFAULT_PORT, help="MQTT Broker Port"
    )
    parser.add_argument(
        "--topic-prefix", default=DEFAULT_PREFIX, help="Prefix for the MQTT topic"
    )
    parser.add_argument(
        "--interval", type=float, default=1.0, help="Average seconds between logs"
    )
