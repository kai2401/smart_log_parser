...
def on_connect(client, userdata, flags, reason_code, properties):
    if reason_code == 0:
        logger.info(
            f"Connected to MQTT broker. Subscribing to topic: {userdata['topic']}"
        )
    ...
    try:
        ...
        for entry in entries:
            if entry.tool_id in (
                ...
                "UNKNOWN_TOOL",
                synthetic_filename,
                synthetic_filename.replace(".log", ""),
            ):
...
def main():
    ...
    parser.add_argument(
        "--broker", default=config.MQTT_BROKER, help="MQTT Broker IP/Hostname"
    )
    parser.add_argument(
        "--port", type=int, default=config.MQTT_PORT, help="MQTT Broker Port"
    )
    parser.add_argument(
        "--topic", default=f"{config.MQTT_TOPIC_PREFIX}/#", help="Topic to subscribe to"
    )
    ...
    client = mqtt.Client(
        mqtt.CallbackAPIVersion.VERSION2, userdata={"topic": args.topic}
    )
